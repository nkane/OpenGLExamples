	...
	if (FileHandle != INVALID_HANDLE_VALUE) 
	{
		...
		if (GetFileSizeEx(FileHandle, &FileSize))
		{
			...
			if (Result.Contents)
			{
				...
				{
					// TODO(nick): write free file memory here ...
					Result.Contents = 0;
				}
			}
		}
...
// TODO(nick): allocate memory for BMP file
loaded_bitmap * LoadBMP(char *FileName)
{
	// TODO(nick): maybe change to a global? need a better way to have it released
...
int LoadGLTextures()
{
	...
	if (TextureImage[0] = LoadBMP("../Data/cube.bmp"))
	{
		...
		glBindTexture(GL_TEXTURE_2D, texture[0]);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, TextureImage[0]->Width, TextureImage[0]->Height, 0, GL_RGBA,
